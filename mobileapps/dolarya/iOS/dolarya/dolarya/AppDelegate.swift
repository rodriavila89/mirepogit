...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        self.client = MobileServiceHelper.GetInstance().GetClient()
    ...
    func application(application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: NSData) {
        ...
        MobileServiceHelper.GetInstance().deviceToken = deviceToken
        self.client!.push.registerNativeWithDeviceToken(deviceToken, tags: categories, completion: {(error) -> Void in
            if error != nil{
                self.client!.push.registerNativeWithDeviceToken(deviceToken, tags: categories, completion: {(error) -> Void in
                    if error != nil{
                        self.client!.push.registerNativeWithDeviceToken(deviceToken, tags: categories, completion: {(error) -> Void in
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "com.spirit.Test" in the application's documents Application Support directory.
    ...
    func setupAppearance()
    {
        ...
        let image = UIImage(named: "ab_background")!.resizableImageWithCapInsets(UIEdgeInsetsMake(0, 0, 0, 0), resizingMode: UIImageResizingMode.Stretch)
